...
file_path = 'result/cifar100/resnet18/r18_ls0_LR3e-2_sv/ep499.pickle'

with open(file_path, 'rb') as f:
    out = pickle.load(f)

def get_bn_dict(out, num_bins):
...
bin_dict_pre, bin_dict_post = get_bn_dict(out, num_bins=num_bins)
bns = [(i / float(num_bins)) for i in range(num_bins)]


# ====== plot acc
def get_values(bin_dict, name):
    num_samples = sum([bin['count'] for k, bin in bin_dict_pre.items()])
    y = []
    if name in ['acc']:
        for i in range(num_bins):
...
acc_pre = get_values(bin_dict_pre, 'acc')
acc_post = get_values(bin_dict_post, 'acc')
...
axes[0].bar(positions1, np.array(acc_pre)/100, width=bar_width, label='CE', color='blue', alpha=0.5)
axes[0].legend()
...
axes[1].bar(positions1, np.array(acc_post)/100, width=bar_width, label='CE', color='blue', alpha=0.5)
axes[1].legend()
